...
function unique_contact_number($contact_number){
  ...
  global $db;
  $sql = "SELECT contact_number FROM hospital_list WHERE contact_number='".$contact_number."'";
  $check = $db->query($sql);
...
function register($name,$password,$contact_number,$address){

   global $db;
   $sql="INSERT INTO hospital_list(hospital_name,password,contact_number,address) VALUES(?,?,?,?)";
   $query=$db->prepare($sql);
   $query->bind_param('ssss',$name,$password,$contact_number,$address);
   $exec= $query->execute();
    if($exec==true)
    {
