...
$register=$valid=$nameErr=$contactErr=$passErr=$cpassErr='';
 // by default,set input values are empty
 $set_name=$set_contactnumber='';
...
if(empty($name)){
   $nameErr="Vehicle Number is Required"; 
}else{
   $nameErr=true;
}
...
if($nameErr==1 && $contactErr==1 && $passErr==1 && $cpassErr==1)
{
    ...
    if($checkContact)
    {
      $register=$contact_number." is already exist";
       ...
       // Insert data
      $register=register($name,$password,$contact_number);
...
function unique_contact_number($contact_number){
  ...
  global $db;
  $sql = "SELECT contact_number FROM ambulance_list WHERE contact_number='".$contact_number."'";
  $check = $db->query($sql);
...
function register($name,$password,$contact_number){

   global $db;
   $sql="INSERT INTO ambulance_list(vehicle_number,password,contact_number) VALUES(?,?,?)";
   $query=$db->prepare($sql);
   $query->bind_param('sss',$name,$password,$contact_number);
   $exec= $query->execute();
    if($exec==true)
    {
